function Invoke-KioskGitHubUpdate {
    ...
    if (-not $ManifestUrl) {
        $ManifestUrl = "https://raw.githubusercontent.com/masas841/Sber2026/main/$Subdir/install/update-manifest.json"
    }
    if (-not $RawBaseUrl) {
        $RawBaseUrl = Read-DotEnvValue -Path $envPath -Name "UPDATE_RAW_BASE_URL"
    }
    if (-not $RawBaseUrl) {
        $RawBaseUrl = "https://raw.githubusercontent.com/masas841/Sber2026/main/$Subdir"
    }
