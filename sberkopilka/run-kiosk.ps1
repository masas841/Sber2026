...
if (-not $SkipUpdate -and ($UpdateFromGitHub -or $autoUpdate)) {
    ...
    if (Test-Path $updater) {
        Write-Host "[SberKopilka] Checking GitHub update ..." -ForegroundColor Cyan
        & $updater
    }
}
...
if (-not (Test-Path $venvPy)) {
    ...
    Write-Host "[SberKopilka] Creating .venv from $basePy ..."
...
if (Test-Path $envDefaults) {
    . $envDefaults
    Update-SberKopilkaEnvDefaults -Root $PSScriptRoot
}
...
Write-Host "SberKopilka: http://127.0.0.1:$port  (podklyuchite dzhojstik)" -ForegroundColor Cyan
python -m app.main
