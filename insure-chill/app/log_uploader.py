...
def _base_url() -> str | None:
    url = (settings.log_upload_url or "").strip().rstrip("/")
    if not url:
        return None
...
def _headers() -> dict[str, str]:
    key = (settings.log_upload_api_key or "").strip()
    if not key:
        return {}
    mode = (settings.log_upload_auth or "bearer").strip().lower()
    if mode == "x-api-key":
...
def start_log_upload_worker() -> None:
    ...
    base = _base_url()
    if not base:
        logger.warning("log_upload enabled, but LOG_UPLOAD_URL is empty")
        return
    with _worker_lock:
        if _worker_started:
            return
