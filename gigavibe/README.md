...
Пороги профиля гостя в `.env`: `GUEST_AGE_CHILD_MAX=15`, `GUEST_AGE_SENIOR_MIN=55` (между — взрослый). Для группового селфи вторым гостем считается только крупное центральное лицо: `GUEST_FACE_MIN_RELATIVE_SIZE=0.35`, `GUEST_FACE_CENTER_WIDTH=0.72`, `GUEST_FACE_CENTER_HEIGHT=0.86`; мелкие прохожие у края кадра игнорируются.
