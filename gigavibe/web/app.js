...
let kioskCfg = {
  ...
  kiosk_face_min_size: 0.06,
  kiosk_face_hold_frames: 12,
  kiosk_face_release_frames: 20,
  kiosk_face_hold_ms: 350,
  kiosk_face_release_ms: 2500,
  kiosk_face_detect_stride: 10,
  kiosk_face_detection_confidence: 0.04,
  kiosk_detection_focus_y: 0.22,
  kiosk_detection_zoom: 1.65,
  print_enabled: false,
...
function prepareDetectionFrame() {
  ...
  drawVideoFit(
    ...
    DETECTION_FRAME_H,
    {
      zoom: kioskCfg.kiosk_detection_zoom ?? CAMERA_ZOOM,
...
async function startPresenceWatch() {
  ...
  try {
    ...
    presenceWatcher = await createFacePresenceWatcher(detectionSource, {
      ...
      detectionConfidence: kioskCfg.kiosk_face_detection_confidence,
