...
После обновления updater запускает миграцию `.env`: если новых ключей нет, они дописываются
с безопасными значениями по умолчанию. Уже заданные значения, включая API-ключи и
`LOG_UPLOAD_ENABLED`, не перетираются.
