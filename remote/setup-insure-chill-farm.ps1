...
$envSrc = Join-Path $remote "farm-insure-chill.env"
if (Test-Path $envSrc) {
    Copy-Item $envSrc (Join-Path $proj ".env") -Force
    Write-Host "[setup] .env from farm-insure-chill.env"
}

& netsh advfirewall firewall delete rule name=InsureChill-8768 2>&1 | Out-Null
& netsh advfirewall firewall add rule name=InsureChill-8768 dir=in action=allow protocol=TCP localport=8768 | Out-Null
Write-Host "[setup] Firewall rule for TCP 8768"

& powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $remote "register-insure-chill-task.ps1")
